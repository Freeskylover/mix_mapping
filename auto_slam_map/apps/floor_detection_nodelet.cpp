...
#include <auto_slam_map/FloorCoeffs.h>
...
public:
  ...
  virtual void onInit() {
    ...
    points_sub = nh.subscribe("/filtered_points", 256, &FloorDetectionNodelet::cloud_callback, this);
    floor_pub = nh.advertise<auto_slam_map::FloorCoeffs>("/floor_detection/floor_coeffs", 32);
...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    auto_slam_map::FloorCoeffs coeffs;
  ...
  boost::optional<Eigen::Vector4f> detect(const pcl::PointCloud<PointT>::Ptr& cloud) const {
    ...
    pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients);///可以直接代替coeffs(再说吧，应该可以)
    ...
    seg.segment(*inliers_floor,*coefficients);
    //====12.03
    ...
    if(floor_points_pub.getNumSubscribers()) {
      ...
      extract.setIndices(inliers_floor);//12.03
    ...
    if(floor_filtered_pub.getNumSubscribers()) {
      ...
      pcl::ExtractIndices<PointT> extract;
      extract.setInputCloud(cloud);//tilt_deg!=0时可能会出问题
      extract.setIndices(inliers_floor);
      extract.setNegative(true);
      extract.filter(*outlier_cloud);
      outlier_cloud->header = cloud->header;
      floor_filtered_pub.publish(outlier_cloud);
    }
