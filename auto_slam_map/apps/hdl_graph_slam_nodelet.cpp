...
#include <auto_slam_map/SaveMap.h>
#include <auto_slam_map/DumpGraph.h>
...
class AutoSlamNodelet : public nodelet::Nodelet {
public:
  typedef pcl::PointXYZI PointT;

  AutoSlamNodelet() {}
  virtual ~AutoSlamNodelet() {}
  ...
  virtual void onInit() {
    ...
    sensor_height = private_nh.param<double>("sensor_height", 0.0);//11.27
    height_clip_min = private_nh.param<double>("height_clip_min", -0.2);//11.27
    height_clip_max = private_nh.param<double>("height_clip_max", 0.4);//11.27

    // subscribers
    odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(mt_nh, "/odom", 256));
    //cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, "/filtered_points", 32));//11.27
    //cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, points_topic, 32));//11.27/segmented_cloud
    cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, "/segmented_cloud", 32));//11.27/segmented_cloud
    sync.reset(new message_filters::TimeSynchronizer<nav_msgs::Odometry, sensor_msgs::PointCloud2>(*odom_sub, *cloud_sub, 32));
    sync->registerCallback(boost::bind(&AutoSlamNodelet::cloud_callback, this, _1, _2));
    floor_sub = nh.subscribe("/floor_detection/floor_coeffs", 1024, &AutoSlamNodelet::floor_coeffs_callback, this);//11.27

    if(private_nh.param<bool>("enable_gps", true)) {
      gps_sub = mt_nh.subscribe("/gps/geopoint", 1024, &AutoSlamNodelet::gps_callback, this);
      // nmea_sub = mt_nh.subscribe("/gpsimu_driver/nmea_sentence", 1024, &AutoSlamNodelet::nmea_callback, this);
      navsat_sub = mt_nh.subscribe("/fix", 1024, &AutoSlamNodelet::navsat_callback, this);//12.02
    }
    ...
    dump_service_server = mt_nh.advertiseService("/auto_slam_map/dump", &AutoSlamNodelet::dump_service, this);
    save_map_service_server = mt_nh.advertiseService("/auto_slam_map/save_map", &AutoSlamNodelet::save_map_service, this);

    double graph_update_interval = private_nh.param<double>("graph_update_interval", 3.0);
    double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 10.0);
    optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval), &AutoSlamNodelet::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &AutoSlamNodelet::map_points_publish_timer_callback, this);
  }
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    pcl::PointCloud<PointT>::Ptr clipped_cloud(new pcl::PointCloud<PointT>());
    ...
    clipped_cloud = plane_clip(cloud, Eigen::Vector4f(0.0f, 0.0f, 1.0f, sensor_height + height_clip_max), false);//11.27
    clipped_cloud = plane_clip(clipped_cloud, Eigen::Vector4f(0.0f, 0.0f, 1.0f, sensor_height + height_clip_min), true);//11.27
    KeyFrame::Ptr clipped_keyframe(new KeyFrame(stamp, odom, accum_d, clipped_cloud));//11.27
    clipped_keyframes.push_back(clipped_keyframe);//11.27    
  ...
  void navsat_callback(const sensor_msgs::NavSatFixConstPtr& navsat_msg) {
    if((int)navsat_msg->status.status < 4 ){//11.28 gps数据不准确，则不输入//12.2
      return;
    }
    geographic_msgs::GeoPointStampedPtr gps_msg(new geographic_msgs::GeoPointStamped());
    ...
    std::string heading_str;std::stringstream ss;ss<<std::setprecision(10)<<navsat_msg->position_covariance[0];ss>>heading_str;//12.02
    gps_msg->header.frame_id = heading_str;//12.02
    gps_callback(gps_msg);
  }
  ...
  bool flush_gps_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      //=========12.02
      std::istringstream iss((*closest_gps)->header.frame_id);
      float gps_heading; iss >> gps_heading;
      std::cout << gps_heading << std::endl;
      tf::Quaternion gps_quat(0,0,sin(gps_heading/2),cos(gps_heading/2));tf::Vector3 gps_p(utm.easting,utm.northing,utm.altitude);
      tf::Transform trans(gps_quat,gps_p);
      tf::StampedTransform transform;
      tf_listener.waitForTransform(base_frame_id, "gps_link", ros::Time(0), ros::Duration(2.0));
      tf_listener.lookupTransform(base_frame_id, "gps_link", ros::Time(0), transform);
      Eigen::Vector3d xyz((transform.inverse() * trans).getOrigin().getX(), (transform.inverse() * trans).getOrigin().getY(), (transform.inverse() * trans).getOrigin().getZ());
      //std::cout<< trans.getOrigin().getX()<<" "<<trans.getOrigin().getY()<<" "<<trans.getOrigin().getZ()<<std::endl;
      //std::cout<< (transform.inverse() * trans).getOrigin().getX()<<" "<<(transform.inverse() * trans).getOrigin().getY()<<" "<<(transform.inverse() * trans).getOrigin().getZ()<<std::endl;
      //std::cout<<"========================="<<std::endl;
      //==============12.02
      ...
      if(!zero_utm) {
        zero_utm = xyz;
        std::cout << std::setprecision(10) << *zero_utm << std::endl;//12.02
      }
      xyz -= (*zero_utm);
   ...
   */
  void floor_coeffs_callback(const auto_slam_map::FloorCoeffsConstPtr& floor_coeffs_msg) {
  ...
  bool flush_floor_queue() {
    ...
    auto remove_loc = std::upper_bound(floor_coeffs_queue.begin(), floor_coeffs_queue.end(), latest_keyframe_stamp,
      [=](const ros::Time& stamp, const auto_slam_map::FloorCoeffsConstPtr& coeffs) {
  ...
  void map_points_publish_timer_callback(const ros::WallTimerEvent& event) {
    ...
    auto cloud = map_cloud_generator->generate(snapshot, clipped_keyframes, 0.05, true);//11.27
    if(!cloud) {
      return;
    }
   ...
   * @return
   */
  bool dump_service(auto_slam_map::DumpGraphRequest& req, auto_slam_map::DumpGraphResponse& res) {
    std::lock_guard<std::mutex> lock(main_thread_mutex);
    ...
    //static std::ofstream ofs(directory + "guiji.txt");
    for(int i=0; i<keyframes.size(); i++) {
       std::stringstream sst;
       sst << boost::format("%s/%06d") % directory % i;

       keyframes[i]->dump(sst.str());
      //Eigen::Isometry3d transform(keyframes[i]->node->estimate().matrix());
      //ofs<<i<<" "<< transform.translation().transpose()<<std::endl;
    }
   ...
   * @return
   */
  bool save_map_service(auto_slam_map::SaveMapRequest& req, auto_slam_map::SaveMapResponse& res) {
    std::vector<KeyFrameSnapshot::Ptr> snapshot;
    ...
    auto cloud = map_cloud_generator->generate(snapshot, clipped_keyframes, req.resolution, req.mode);//11.27    
    if(!cloud) {
      res.success = false;
      return true;
    }
...
private:
  ...
  std::vector<KeyFrame::Ptr> clipped_keyframes;//11.27
  double sensor_height;//11.27
  double height_clip_min;//11.27
  double height_clip_max;//11.27
  ...
  std::deque<auto_slam_map::FloorCoeffsConstPtr> floor_coeffs_queue;
...
PLUGINLIB_EXPORT_CLASS(hdl_graph_slam::AutoSlamNodelet, nodelet::Nodelet)
